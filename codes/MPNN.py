...
class MPNNModel(nn.Module):
    def __init__(self,
        ...
        # 最后一个MPNN层
        edge_net_last = nn.Sequential(
            nn.Linear(num_edge_features, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, hidden_dim * hidden_dim)
        )
        self.conv_last = NNConv(
            hidden_dim,
            hidden_dim,
            edge_net_last,
            aggr='mean'
...
def train_mpnn_model(model, train_loader, val_loader, device, num_epochs=100, lr=0.001):
    optimizer = Adam(model.parameters(), lr=lr, weight_decay=1e-5)
    criterion = nn.BCELoss()

    # 添加学习率调度器 - 移除 verbose 参数
    ...
    for epoch in range(num_epochs):
        ...
        if len(np.unique(all_targets)) > 1:  # 确保有正负样本
...
# 评估函数
def evaluate_mpnn_model(model, test_loader, device):
    model.eval()
    all_preds = []
    all_targets = []
    all_smiles = []
...
def save_mpnn_model(model, training_config, filepath):
    """
    保存MPNN模型和训练配置
    """
    try:
        ...
        print(f"保存失败: {e}")
        try:
            torch.save(model.state_dict(), "mpnn_weights_backup.pth")
            print("模型权重已备份保存到: mpnn_weights_backup.pth")
            return True
        except Exception as e2:
            print(f"备用保存也失败: {e2}")
            return False

def load_mpnn_model(filepath, device=None):
    """
    加载MPNN模型
    """
    if device is None:
    ...
    print(f"MPNN模型已从 {filepath} 加载")
    print(f"模型配置: {model_config}")
    print(f"最佳验证AUC: {checkpoint['training_config'].get('best_val_auc', 'N/A')}")

    return model, checkpoint

def predict_with_mpnn_model(model, smiles_list, device=None):
    """
    使用加载的GCN模型进行预测
    """
    if device is None:
    ...
    with torch.no_grad():
        for smiles in smiles_list:
            ...
            # 预测
            output = model(data)
            prediction = output.cpu().numpy()[0][0]  # 获取标量预测值
...
def main():
    ...
    print(f'MPNN模型创建完成，参数数量: {sum(p.numel() for p in model.parameters())}')

    # 训练模型
    training_results = train_mpnn_model(
        model, train_loader, val_loader, device,
        num_epochs=20, lr=0.001
    )

    # 评估模型
    test_results = evaluate_mpnn_model(model, test_loader, device)

    print(f'MPNN Test Results:')
    ...
    save_mpnn_model(model, training_config, r"D:\Research\ML_data\colorectal_cancer\models\best_mpnn_model.pth")
